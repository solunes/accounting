...
class Business {
    ...
    public static function check_report_header($model, $places = [], $extra = NULL) {
        ...
        $currency = \Solunes\Accounting\App\Currency::find($currency_id);
        $currencies = \Solunes\Accounting\App\Currency::where('in_accounts', 1)->get()->lists('real_name', 'id');
        ...
        if($extra&&$extra=='account_id'){
            ...
            $array['accounts'] = \Solunes\Accounting\App\Account::lists('name','id');
            $array['current_account_id'] = 1;
            if(request()->has('account_id')){
                $array['current_account_id'] = request()->input('account_id');
            }
        }
        $array['places'] = ['all'=>'Consolidado']  + \Solunes\Accounting\App\Place::lists('name', 'id')->toArray() + $places;
